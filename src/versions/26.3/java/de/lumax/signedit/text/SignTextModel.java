...
public class SignTextModel {
    ...
    public SignText buildSignText() {
        return buildSignText(SignText.EMPTY);
    }

    public SignText buildSignText(SignText baseText) {
        SignText.Mutable result = baseText.asMutable();
    ...
    public void loadFromSignText(SignText signText, boolean filtered) {
        for (int line = 0; line < 4; line++) {
            Component component = signText.getMessages(filtered).get(line);
